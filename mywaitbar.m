...
%   $Revision: 1.23.4.3 $  $Date: 2004/08/16 01:54:58 $
...
switch type
    case 1,  % waitbar(x)    update
        ...
        mine=floor(tme/60.0);
        sece=tme-mine*60;
        xttl=sprintf('%3.f%% done;  Time: spent=%3d''%2.f" left=%3d''%2.f"',x,mine,sece,minl,secl);
        set(get(ax,'xlabel'),'string',xttl,'FontName','Courier');
    ...
    case 2,  % waitbar(x,name)  initialize
        ...
        pos=screenSize;
        pos = [pos(3)-width-7*pointsPerPixel pos(4)-height-28*pointsPerPixel width height];
